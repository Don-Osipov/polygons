...
def draw_polygons(polygons, screen, color):
    if len(polygons) < 3:
        print("Cannot draw polygon with less than 3 points")
        return

    for point in range(0, len(polygons), 3):
        normal = calculate_normal(polygons, point)
        theta = dot_product(normal, [0, 0, 1])
...
def addTri1(a, b, c, d, points, polygons):
    add_polygon(
        polygons,
        points[a][0],
        points[a][1],
...
def addTri2(a, b, c, d, points, polygons):
    add_polygon(
        polygons,
        points[a][0],
        points[a][1],
...
def add_sphere(polygons, cx, cy, cz, r, steps):
    ...
    for lat in range(lat_start, lat_stop):
        for longt in range(longt_start, longt_stop + 1):
            ...
            args = (a, b, c, d, points, polygons)

            addTri1(*args) if longt != 0 and longt != (steps - 1) else addTri2(*args)
